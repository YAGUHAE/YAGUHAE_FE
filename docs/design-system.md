...
| 7 | `SegmentedTab` | Selected(T/F) | 2 | P-7 상태 탭, P-4·P-5·A-5 팀 탭, **A-7 팀 탭** |
...
| A-7 출석 체크 | 1148 | **선공/후공 팀 탭**으로 분리. 한 팀 라인업 11행 + 탭 아래 두 팀 진행률 한 줄. 대리 신청분에 `대리` 배지. 아래 별도 항목 |
...
### A-7 출석 체크의 팀 탭 (2026-08-23, 명세 v2.1)

어드민 화면 중 A-7만 선공/후공이 갈라지지 않은 채 두 팀 22명을 한 리스트에 이어 붙이고 있었습니다. 명세가 근거로 든 "어드민이 보는 건 라인업"인데, 야구장의 라인업은 팀별로 존재합니다. A-5(= 용병 P-4)와 같은 `SegmentedTab`을 얹어 한 번에 한 팀만 보여줍니다.

- **`SegmentedTab`을 고치지 않았습니다.** 탭 라벨에 팀별 진행률을 2줄로 넣으려 했지만 `LabelWrap` 높이가 `size/touch-min`에 바인딩돼 2줄이 들어가지 않고, 라벨 TEXT 프로퍼티 하나를 쓰는 인스턴스가 P-4·P-5·P-7에 7개 살아 있습니다. 공용 컴포넌트를 건드리는 대가가 A-7 하나가 얻는 것보다 큽니다
- **대신 탭 바로 아래 진행률 한 줄**(`선공 체크 11/11 · 후공 체크 5/11`)을 뒀습니다. 탭 라벨에 넣는 것보다 오히려 낫습니다 — **어느 탭을 보고 있든 두 팀 숫자가 동시에** 보입니다. "미체크 인원이 반대 탭에 숨는다"는 이 화면의 유일한 실질 위험을 막는 장치가 이것입니다
- **`체크`라는 낱말이 분모를 구분합니다.** A-2·A-3·A-5의 `선공 5/11`은 충원 현황(찬 자리 / 슬롯 수)이고, 여기 `11/11`은 체크 진행률(체크한 사람 / `APPROVED` 인원)입니다. 같은 포맷을 그대로 쓰면 섞여 읽힙니다. A-4의 `선공 11자리 · 후공 11자리`가 이미 쓰는 방식입니다
- **숫자에 Roboto Mono를 쓰지 않았습니다.** §6 구성 원칙의 "숫자는 전부 모노"에 걸리는 것처럼 보이지만, 이 파일의 `선공 N/11 · 후공 N/11` 표기는 A-2(Caption)·A-3(Body/Small)·A-5(Body/Medium)·A-4까지 **전부 본문 서체 단일 스타일**입니다. 한글에 붙은 인라인 카운트라 모노를 섞으면 같은 원칙의 "한글 값에는 쓰지 않습니다" 쪽과 충돌합니다. 실제로 `setRangeFontName`으로 숫자만 모노를 걸었더니 `textStyleId`가 `mixed`로 풀려 §7의 "텍스트 스타일 미적용 0"이 깨졌습니다 → §9-19
- **미체크 안내는 `TextLink` 인스턴스이고 fill을 `text/brand`로 오버라이드**했습니다(컴포넌트 기본값은 `text/secondary`). 링크가 아니라 **반대 탭으로 보내는 진입점**이라 `PositionSlot` `Empty`의 "신청 가능"과 같은 취급입니다. 비활성 버튼 라벨에 사유를 넣던 방식(`2명 더 체크하면 제출할 수 있어요`)은 되돌리고 버튼을 `제출`로 두었습니다 — **비활성 버튼은 탭 타깃이 될 수 없는데 이 문구는 탭해서 이동해야 합니다**
- 라인업을 **11행 전부**로 채웠습니다(좌익·우익 추가). 이전 목업은 높이를 줄이려고 8·10번을 빼고 9행만 뒀는데, 진행률에 `11/11`이라고 쓰는 이상 행이 9개면 목업이 스스로 모순됩니다. 프레임 1000 → 1148
- **신규 컴포넌트 0개, 신규 토큰 0개.** 대비 실측 — 진행률 `text/secondary` 7.47, 미체크 안내 `text/brand` 6.57. 자동 검사 3종(넘친 노드 · 미바인딩 fill · 텍스트 스타일 미적용) 전부 0
